module mod_get_daisy_temperatures


contains

subroutine get_daisy_temperatures(n_times,year,month,day,t,missing_value,TAverage,doy_max,TAmplitude)
use mod_leap_year
...
logical:: lp, year_valid, leap_day
...
do i_years = 1, n_years
  ...
  if(start_index.ge.1)then
    ...
    enddo
    lp = leap_year(rec_year)
    year_valid = .false.
    if(lp)then
      if(n_valid.eq.366)then
        year_valid = .true.
      endif
     else
...
do i_times = 1, n_times
  leap_day = .false.
  if(month(i_times).eq.2)then
    if(day(i_times).eq.29)then
      leap_day = .true.
    endif
  endif
  if(.not.leap_day)then
    if(abs(t(i_times)-mv).gt.eps)then
      n = n + 1
    endif  
  endif  
...
end subroutine get_daisy_temperatures

end module mod_get_daisy_temperatures
