module mod_check_availability_of_parameter
...
logical function check_availability_of_parameter(dwf,param,missing_value)
...
i = find_string_in_array(dwf%n_vars, dwf%varnames, trim(adjustl(param)))
if(i.lt.0)then
  check_availability_of_parameter = .false.
 else
  check_availability_of_parameter = .false. 
  found = .false.
  i_times = 1
  do 
    ...
    if(abs(dwf%var(i_times,i)-missing_value).gt.eps)then
      check_availability_of_parameter = .true.
      found = .true. 
     else
      i_times = i_times + 1
    endif
